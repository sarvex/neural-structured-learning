...
def add_variable_summaries(var, var_name_scope):
  ...
  with tf.name_scope(f'summaries/{var_name_scope}'):
...
def read_entity_name_mapping(entity_names_file):
  ...
  with open(entity_names_file) as gf:
    f = gzip.GzipFile(fileobj=gf) if entity_names_file.endswith('.gz') else gf
    for line in f:
